# Generate another synthetic fashion apparel dataset for testing inserts
import random
import pandas as pd
import numpy as np
import datetime

random.seed(123)   # different seed to make new data
np.random.seed(123)

# Configuration
num_products = 10   # number of new products
categories = [
...
for i in range(1, num_products + 1):
    ...
    cost = random.randint(5, 30) * 10
    price = cost + random.randint(2, 7) * 10
    starting_inventory = random.randint(5, 100) * 5

    products.append({
        "master_product_code": "SKU_NEW"+f"{i:04d}",  # distinguish from old batch
        "name": name,
        "category": category,
        "brand": brand,
...
df_new.to_csv("data/master_product_new.csv", index=False)
print("✅ Generated master_product_new.csv")
